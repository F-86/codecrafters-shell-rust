...
//!   保留；`$NAME` 触发变量展开（命中替换为值，未命中贡献空串但 token 仍由
//!   引号开启故保留为空 token），`\$` 提前消费
...
//!   到第一个非合法字符为止，命中 `vars` 替换为值（即便值为空串也开启 word）；
//!   未命中不贡献任何字符且不开启 word——若整个 word 完全由 unquoted 未命中
//!   展开贡献，整个 word 在 flush 时被丢弃（bash 的「null word removal」），
//!   但若 word 含字面字符 / 引号片段 / 转义字符 / 命中展开（即便值为空串）则
//!   始终保留；`$` 后非合法首字符（如 `$1abc` / `$-` / `$<空白>` / 行尾孤立 `$`）
//!   按字面量保留 `$`；
//! - 引号外与双引号内还支持 `${NAME}` 大括号形式：扫描到 `}` 闭合后对内部字符串
//!   做整串 NAME 校验（同 `$NAME` 字符集，与 `is_name_start`/`is_name_cont` 同源），
//!   命中替换为值并开启 word、未命中不开启 word（unquoted 全空 word 被丢弃，
//!   双引号内由引号片段开启故仍保留为空 token）。大括号边界明确，可让 `${X}end`
//!   拼接字面 `end` 而无需空白分隔。错误形式：内部 NAME 非法（空 `${}`、
//!   首字符非法 `${1abc}`、含非 NAME 字符 `${X-Y}` 等）→ `BadSubstitution`；
//!   行尾未见闭合 `}` → `UnterminatedBraceExpansion`。`\${X}` 在 Normal 与双引号
//!   态均复用反斜杠路径——`$` 被先消费为字面 push，下一轮主循环看到 `{`
//!   字面字符，自然得到 `${X}` 字面输出；
